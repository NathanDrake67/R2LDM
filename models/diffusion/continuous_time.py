...
class ContinuousTimeGaussianDiffusion(base.GaussianDiffusion):
    ...
    def get_network_condition(self, steps, cond):
    ...
    def q_step_from_x_0(self, x_0, step_t, rng=None):
        # Forward diffusion q(z_t | x_0), where 0 < t < 1.
    ...
    def p_step(
        ...
        # Reverse diffusion p(z_s | z_t), where 0 < s < t < 1.
        log_snr_t = self.log_snr(step_t)
        log_snr_s = self.log_snr(step_s)
        alpha_t, sigma_t = _log_snr_to_alpha_sigma(log_snr_t)
        alpha_s, sigma_s = _log_snr_to_alpha_sigma(log_snr_s)
        if condition is None:
            ...
            prediction = self.model(x_t, log_snr_t[:, 0, 0, 0], condition)
    ...
    def cond_generate(
        ...
        x_t = self.randn(batch_size, *(8, 80, 80), rng=rng, device=self.device)
        ...
        for i in tqdm(
            ...
            for j in range(num_resample_steps):
                ...
                for k in range(jump_length):
                    ...
                    unknown_s = self.p_step(
                        x, r_step_t, r_step_s, condition, rng=rng
                    )
