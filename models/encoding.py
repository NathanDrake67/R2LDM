"""Coordinate encodings used by the diffusion network."""
...
def components_from_spherical_harmonics(levels: int, directions):
    """Return spherical-harmonic components for XYZ direction vectors."""
    ...
    assert directions.shape[-1] == 3, (
        "Direction input should have three dimensions. "
...
class SphericalHarmonics(nn.Module):
    ...
    def forward(self, coords: torch.Tensor) -> torch.Tensor:
        batch_size, _, _, _ = coords.shape
        coords = coords.repeat_interleave(batch_size, dim=0)
        ...
        return basis.permute(0, 3, 1, 2).repeat_interleave(
            batch_size, dim=0
        )
...
class FourierFeatures(nn.Module):
    ...
    def forward(self, coords: torch.Tensor) -> torch.Tensor:
        ...
        return torch.cat([coords.sin(), coords.cos()], dim=1)

    def extra_repr(self):
        return (
            f"shape={self.resolution}, num_freqs={self.extra_ch}, "
            f"L=({self.L_h}, {self.L_w})"
        )
