...
class ResidualBlock(nn.Module):
    def __init__(
        ...
        self.conv2 = ops.Conv2d(out_channels, out_channels, 3, 1, 1, ring=ring) # 512 512
...
class Block(nn.Module):
    ...
    def forward(
        self, h: torch.Tensor, temb: torch.Tensor | None = None # h(1,64,64,1024)
    ) -> torch.Tensor:
        h = self.downsample(h)
...
class EfficientUNet(nn.Module):
    ...
    def __init__(
        ...
        self.in_conv_1 = ops.Conv2d(16, 32, 3, 1, 1, ring=ring)

        self.in_conv = ops.Conv2d(32, C[0], 3, 1, 1, ring=ring)
        ...
        self.d_block4 = Block(C[3], C[4], N[3], down=2, **cfgs)

        # upsampling blocks
        # self.u_block4 = Block(C[4], C[3], N[3], up=2, attn=True, **cfgs)# Block(2048,1024,3)
        self.u_block4 = Block(C[4], C[3], N[3], up=2, **cfgs)  # Block(2048,1024,3)
        self.u_block3 = Block(C[3] + C[3], C[2], N[2], up=2, **cfgs)# Block(1024+1024,512,3)
        self.u_block2 = Block(C[2] + C[2], C[1], N[1], up=2, **cfgs)# Block(512+512,256,3)
        self.u_block1 = Block(C[1] + C[1], C[0], N[0], **cfgs)# Block(256+256,256,3)
        self.out_conv = ops.Conv2d(C[0], 32, 3, 1, 1, ring=ring)  # ops.Conv2d(256,None)
        #self.out_conv = ops.Conv2d(C[0], self.out_channels, 3, 1, 1, ring=ring) # ops.Conv2d(256,None)
        self.out_conv_1 = ops.Conv2d(32, 8, 3, 1, 1, ring=ring)
        self.out_conv.apply(ops.zero_out)

    def forward(self, images: torch.Tensor, timesteps: torch.Tensor , radar_cond: torch.Tensor | None = None) -> torch.Tensor:
        h = images
        ...
        if radar_cond is not None:
            h = torch.cat([h, radar_cond], dim=1)

        h = self.in_conv_1(h)           # h  (1,32,64,64)
        h = self.in_conv(h)             # h  (1,64,64,64)
        h1 = self.d_block1(h, temb)     # h1 (1,64,64,64)
        h2 = self.d_block2(h1, temb)    # h2 (1,128,32,32)
        h3 = self.d_block3(h2, temb)    # h2 (1,256,16,16)
        h4 = self.d_block4(h3, temb)    # h4 (1,512,8,8)
        h = self.u_block4(h4, temb)                     # (1,256,16,16)
        h = self.u_block3(_join(h, h3), temb)  # (1,128,32,32)
        h = self.u_block2(_join(h, h2), temb)  #  h (1,64,64,64)
        h = self.u_block1(_join(h, h1), temb)  #  h (1,64,64,64)
        h = self.out_conv(h)   # h (1,32,64,64)
        h = self.out_conv_1(h) # h (1,8,64,64)

        return h    #  h (1,8,64,64)
