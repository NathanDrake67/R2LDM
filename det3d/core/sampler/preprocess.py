        ...
        for name, min_num in self._min_gt_point_dict.items():
            if min_num > 0:
                ...
                if name not in db_infos:
                    print(f"Warning: Missing {name} in db_infos")
                    continue
...
def global_rotation_(gt_boxes, points, dense_points, completion_points, rotation=np.pi / 4):
    if not isinstance(rotation, list):
        rotation = [-rotation, rotation]
    noise_rotation = np.random.uniform(rotation[0], rotation[1])
    points[:, :3] = box_np_ops.rotation_points_single_angle(
    ...
    dense_points[:, :3] = box_np_ops.rotation_points_single_angle(
        dense_points[:, :3], noise_rotation, axis=2
    )
    completion_points[:,:3] = box_np_ops.rotation_points_single_angle(
        completion_points[:, :3], noise_rotation, axis=2
    )
    gt_boxes[:, :3] = box_np_ops.rotation_points_single_angle(
        gt_boxes[:, :3], noise_rotation, axis=2
    )
    ...
    gt_boxes[:, -1] += noise_rotation
    return gt_boxes, points, dense_points, completion_points
...
def random_flip_both_(gt_boxes, points, dense_points,completion_points,  probability=0.5, flip_coor=None):
    # x flip 
    enable = np.random.choice(
        [False, True], replace=False, p=[1 - probability, probability]
    )
    if enable:
        ...
        gt_boxes[:, -1] = -gt_boxes[:, -1] + np.pi
        points[:, 1] = -points[:, 1]
        dense_points[:, 1] = -dense_points[:,1]
        completion_points[:, 1] = -completion_points[:,1]
        if gt_boxes.shape[1] > 7:  # y axis: x, y, z, w, h, l, vx, vy, r
            gt_boxes[:, 7] = -gt_boxes[:, 7]
    ...
    if enable:
        if flip_coor is None:
            gt_boxes[:, 0] = -gt_boxes[:, 0]
            points[:, 0] = -points[:, 0]
            dense_points[:, 0] = -dense_points[:, 0]
            completion_points[:, 0] = -completion_points[:, 0]
        else:
            gt_boxes[:, 0] = flip_coor * 2 - gt_boxes[:, 0]
            points[:, 0] = flip_coor * 2 - points[:, 0]
            dense_points[:, 0] = flip_coor * 2 - dense_points[:, 0]
            completion_points[:, 0] = flip_coor * 2 - completion_points[:, 0]
        gt_boxes[:, -1] = -gt_boxes[:, -1] + 2 * np.pi
...
def global_scaling_v2_(gt_boxes, points, dense_points, completion_points, min_scale=0.95, max_scale=1.05):
    noise_scale = np.random.uniform(min_scale, max_scale)
    points[:, :3] *= noise_scale
    dense_points[:, :3] *= noise_scale
    gt_boxes[:, :-1] *= noise_scale
    completion_points[:, :3] *= noise_scale
    return gt_boxes, points, dense_points, completion_points
...
def global_translate_(gt_boxes, points,dense_points, completion_points, noise_translate_std):
    """
    Apply global translation to gt_boxes and points.
    """
    ...
    points[:, :3] += noise_translate
    dense_points[:, :3] += noise_translate
    completion_points[:,:3] += noise_translate
    gt_boxes[:, :3] += noise_translate
