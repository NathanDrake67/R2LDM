...
def get_obj(path):
    if os.path.exists(path):
        ...
        path = path.replace("data/Waymo","data/waymo")
        with open(path, 'rb') as f:
                obj = pickle.load(f)
    return obj 
    ...
    def sample_all(
        ...
        if len(sampled) > 0:
            ...
            for info in sampled:
                ...
                if os.path.exists('../data/waymo/train/gt/'+info['gt_signs']+'.pkl'):
                    gt_point = get_obj('../data/waymo/train/gt/'+info['gt_signs']+'.pkl')['gt']
                    if info["name"] == "VEHICLE":
                        ...
                        if ind.sum() > vind.sum():
                            ...
                            vgt_ind[:,1] *= -1
                            gt_point = np.concatenate([gt_point,vgt_ind],0)
                    gt_point[:,:3] = box_np_ops.rotation_points_single_angle(gt_point[:,:3],(np.pi/2+info["box3d_lidar"][-1]),axis=2)
                    gt_point[:,:3] += info["box3d_lidar"][None][:,:3]
                    gt_point = gt_point[np.any(box_np_ops.points_in_rbbox(gt_point, info['box3d_lidar'][None]),axis=1)]
                else:
                    gt_point = s_points
    

                c_points_list.append(gt_point)
            ...
            ret = {
                ...
                "recon_points": np.concatenate(c_points_list, axis=0),
            }
            if self._use_group_sampling:
