...
def boxes3d_to_bev_3d_torch(boxes3d, box_mode='wlh', rect=False):
    ...
    if rect:
        ...
        boxes_bev[:, 0], boxes_bev[:, 1], boxes_bev[:, 2] = cu - half_w, cv - half_l, cw           # left-bottom in velo coord
        boxes_bev[:, 3], boxes_bev[:, 4], boxes_bev[:, 5] = cu + half_w, cv + half_l, cw + height  # right-top in cam coord
    # rz in velo should have the same effect as ry of cam coord in 2D. Points in box will clockwisely rotate with rz/ry angle.
