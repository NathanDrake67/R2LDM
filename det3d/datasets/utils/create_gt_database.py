...
def create_groundtruth_database(
    ...
    **kwargs,
    ...
    pipeline = [
        {
            ...
            "dataset": dataset_name_map[dataset_class_name], #"distillation": True
        },
    ...
    for index in tqdm(range(len(dataset))):
        ...
        # dense_points = sensor_data["lidar"]['dense_points']
        # completion_points = sensor_data["lidar"]['completion_points']
            
        annos = sensor_data["lidar"]["annotations"]
        gt_boxes = annos["boxes"]
        names = annos["names"]
        objects = sensor_data['lidar']['signs']
        ...
        # dense_point_indices = box_np_ops.points_in_rbbox(dense_points, gt_boxes)
        # completion_point_indices = box_np_ops.points_in_rbbox(completion_points, gt_boxes)
        for i in range(num_obj):
            if (used_classes is None) or names[i] in used_classes:
                filename = f"{image_idx}_{names[i]}_{i}.bin"
                # dense_filename = f"{image_idx}_{names[i]}_{i}_dense.bin"
                # completion_filename = f"{image_idx}_{names[i]}_{i}_completion.bin"
                dirpath = os.path.join(str(db_path), names[i])
                os.makedirs(dirpath, exist_ok=True)

                filepath = os.path.join(str(db_path), names[i], filename)
                # dense_filepath = os.path.join(str(db_path), names[i], dense_filename)
                # completion_filepath = os.path.join(str(db_path), names[i], completion_filename)
                gt_points = points[point_indices[:, i]]
                # gt_dense_points = dense_points[dense_point_indices[:, i]]
                # gt_completion_points = completion_points[completion_point_indices[:,i]]
                gt_points[:, :3] -= gt_boxes[i, :3]
                # gt_dense_points[:,:3] -= gt_boxes[i, :3]
                # gt_completion_points[:,:3] -= gt_boxes[i, :3]
                ...
                with open(filepath, "w") as f:
                    try:
                        ...
                        break
                # with open(dense_filepath, "w") as f:
                #     try:
                #         gt_dense_points[ :, :point_features].tofile(f)
                #     except:
                #         break
                # with open(completion_filepath, "w") as f:
                #     try:
                #         gt_completion_points[ :, :point_features].tofile(f)
                #     except:
                #         break
