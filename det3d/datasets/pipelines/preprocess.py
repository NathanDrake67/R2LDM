        ...
        if self.mode == "train":
            ...
            for obj, box, type in zip(objects, info["gt_boxes"], info["gt_names"]):
                if os.path.exists("../data/waymo/train/gt/" + obj + ".pkl") and type != "SIGN":
                    ...
                    gt_point[:, :3] = box_np_ops.rotation_points_single_angle(
                        gt_point[:, :3], np.pi / 2 + box[-1], axis=2
                    )
                ...
                reconstruction_points = reconstruction_points[
                    np.any(
                        box_np_ops.points_in_rbbox(
                            reconstruction_points, info["gt_boxes"]
                        ),
                        axis=1,
                    )
                ]
        ...
        if self.mode == "train" and not self.no_augmentation:
            ...
            if self.db_sampler:
                ...
                if sampled_dict is not None:
                    ...
                    reconstruction_points = np.concatenate(
                        [sampled_recon_points, reconstruction_points], axis=0
                    )
            ...
            gt_dict["gt_boxes"], points, dense_points, reconstruction_points = (
                prep.random_flip_both_(
                    gt_dict["gt_boxes"], points, dense_points, reconstruction_points
                )
            )
            gt_dict["gt_boxes"], points, dense_points, reconstruction_points = (
                prep.global_rotation_(
                    gt_dict["gt_boxes"],
                    points,
                    dense_points,
                    reconstruction_points,
            ...
            point_out_box = ~np.any(
                box_np_ops.points_in_rbbox(points, info["gt_boxes"]), axis=1
            )
            combined_points = [points[point_out_box]]
            reconstruction_points = []
            objects = info["gt_signs"]
...
class Voxelization(object):
    ...
    def __call__(self, res, info):
        ...
        points_tensor = res["lidar"]["points"]
        points_array = np.array(points_tensor)
