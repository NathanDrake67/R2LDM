    ...
    def __call__(self, res, info):
        ...
        if self.mode == "train":
            ...
            if self.distillation:
                ...
                dense_points = np.vstack(combined_points)
                if len(reconstruction_points) == 0:
                    reconstruction_points = np.zeros([1,5])
                else:
                    reconstruction_points = np.vstack(reconstruction_points)
...
class S2D_Voxelization(object):
    ...
    def __call__(self, res):
        ...
        for key in res.keys():
            if key in ['image', 'folder', 'frame_idx']:
                continue
            if res[key]["points"] is None:
                continue

            voxels, coordinates, num_points = self.voxel_generator.generate(
                res[key]["points"], max_voxels=max_voxels 
            )
            ...
            voxels, coordinates, num_points = self.voxel_generator_2.generate(
                res[key]["points"].astype('float32'), max_voxels=max_voxels 
            )

            num_voxels = np.array([voxels.shape[0]], dtype=np.int64)

            res[key]["voxels_2"] = dict(
                voxels=voxels,
                coordinates=coordinates,
                num_points=num_points,
                num_voxels=num_voxels,
...
class S2D_Reformat(object):
    ...
    def __call__(self, res):
        ...
        lidar_voxels = res["lidar"]["voxels"]

        lidar_gt_points = res["lidar_gt"]['points']
        lidar_gt_voxels = res["lidar_gt"]['voxels']
        lidar_gt_voxels_2 = res["lidar_gt"]['voxels_2']
        lidar_gt_voxels_4 = res['lidar_gt']['voxels_4']

        data_bundle = dict(
            lidar_points=lidar_points,
            lidar_voxels=lidar_voxels["voxels"],
            lidar_shape=lidar_voxels["shape"],
            lidar_num_points=lidar_voxels["num_points"],
            lidar_num_voxels=lidar_voxels["num_voxels"],
            lidar_coordinates=lidar_voxels["coordinates"],
            lidar_gt_points=lidar_gt_points,
            lidar_gt_voxels = lidar_gt_voxels['voxels'],
            lidar_gt_coordinates = lidar_gt_voxels["coordinates"],
            lidar_gt_num_voxels = lidar_gt_voxels["num_voxels"],
            lidar_gt_num_points = lidar_gt_voxels['num_points'],
            lidar_gt_voxels_2 = lidar_gt_voxels_2['voxels'],
            lidar_gt_coordinates_2 = lidar_gt_voxels_2["coordinates"],
            lidar_gt_num_voxels_2 = lidar_gt_voxels_2["num_voxels"],
            lidar_gt_num_points_2 = lidar_gt_voxels_2['num_points'],
            lidar_gt_voxels_4 = lidar_gt_voxels_4['voxels'],
            lidar_gt_coordinates_4 = lidar_gt_voxels_4["coordinates"],
            lidar_gt_num_voxels_4 = lidar_gt_voxels_4["num_voxels"],
            lidar_gt_num_points_4 = lidar_gt_voxels_4['num_points']
        )
        if res["radar"]["points"] is not None:
            radar_points = res["radar"]["points"]
            radar_voxels = res["radar"]["voxels"]
            data_bundle.update(dict(
                radar_points=radar_points, 
                radar_voxels=radar_voxels["voxels"],
                radar_shape=radar_voxels["shape"],
                radar_num_points=radar_voxels["num_points"],
                radar_num_voxels=radar_voxels["num_voxels"],
                radar_coordinates=radar_voxels["coordinates"]
                ))
        if res["image"] is not None:
            data_bundle["image"] = res["image"]
        if res["folder"] is not None:
            data_bundle["folder"] = res["folder"]
        if res["frame_idx"] is not None:
            data_bundle["frame_idx"] = res["frame_idx"]
        ...
        if self.double_flip:
            ...
            return [data_bundle, yflip_data_bundle, xflip_data_bundle, double_flip_data_bundle]
