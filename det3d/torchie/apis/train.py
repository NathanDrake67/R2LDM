...
from det3d.core.utils.dist_utils import DistOptimizerHook
from det3d.datasets import DATASETS, build_dataloader
from det3d.solver.fastai_optim import OptimWrapper
from det3d.torchie.trainer import DistSamplerSeedHook, Trainer,TS_Trainer, obj_from_dict
...
def train_TS_detector(T_model, S_model, dataset, cfg, distributed=False, validate=False, logger=None):
    if logger is None:
        logger = get_root_logger(cfg.log_level)
    ...
    total_steps = cfg.total_epochs * len(data_loaders[0])
    if distributed:
        T_model = apex.parallel.convert_syncbn_model(T_model)
        S_model = apex.parallel.convert_syncbn_model(S_model)
    if cfg.lr_config.type == "one_cycle":
        # build trainer
        optimizer = build_one_cycle_optimizer(S_model, cfg.optimizer)
        lr_scheduler = _create_learning_rate_scheduler(
            optimizer, cfg.lr_config, total_steps
        )
        cfg.lr_config = None
    else:
        optimizer = build_optimizer(S_model, cfg.optimizer)
        lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=cfg.drop_step, gamma=.1)
        # lr_scheduler = None
        cfg.lr_config = None 
    # put model on gpus
    if distributed:
        T_model = DistributedDataParallel(
            T_model.cuda(cfg.local_rank),
            device_ids=[cfg.local_rank],
            output_device=cfg.local_rank,
            # broadcast_buffers=False,
            find_unused_parameters=True,
        )
        S_model = DistributedDataParallel(
            S_model.cuda(cfg.local_rank),
            device_ids=[cfg.local_rank],
            output_device=cfg.local_rank,
            # broadcast_buffers=False,
            find_unused_parameters=True,
    ...
    logger.info(f"model structure: {S_model}")
    


    trainer = TS_Trainer(
        T_model, S_model, batch_processor, optimizer, lr_scheduler, cfg.work_dir, cfg.log_level
    )
    ...
    if cfg.resume_from:
        ...
        trainer.load_checkpoint(cfg.load_from)
    if cfg.distillation:
        trainer.load_teacher_checkpoint(cfg.load_from)
        # trainer.load_teacher_checkpoint('./work_dirs/waymo_centerpoint_voxelnet_1x/epoch_12.pth','cuda:{}'.format(torch.cuda.current_device()))
